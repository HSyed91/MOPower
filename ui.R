...
shinyUI(fluidPage(
  ...
  fluidRow(
     column(8,wellPanel(
               helpText("This power calculator is used for the design of multi-omic studies of rare diseases. Simulation of genome and RNA-seq data at the gene-level. Data integration, Power calculation based on power to detect differential gene expression & association with outcome of interrest."),
            # Input: Select a file ----
  ...
  fluidRow(
     column(2,style='padding:0px;',wellPanel(
          ...
          #  numericInput("target", "Power target", value = 90, min = 0, max = 100),
            ...
            sliderInput("obs", "Sample size", min = 1, max = 1000, value = 40),
            conditionalPanel(
  ...
  # False discovery rate page
      tabPanel("FDR & Bonferroni", fluidRow(
        column(8,wellPanel(h5("In sequencing experiments we are fitting one model for each gene/exon/sequence of interest, and therefore performing thousands
               of tests. Need to correct for multiple testing using the Bonferroni correction and calculate the FDR.")))),
               fluidRow(
                 column(8,mainPanel(
    ...
    #  tabPanel("Cost Analysis", "Cost Analysis based on simulated study design."),
      ...
      tabPanel("About", 
               ...
               with a significant association for total effects of omics datasets with the outcome of interest (case-control or survival). See vignette for more details: https://github.com/HSyed91/MOPower"
                 )))),
